...
import { GlassCard, TactileButton } from '../ui/TactilePrimitives';
...
export const ResultView: React.FC<{ result: ExecutionResult; onReset: () => void }> = ({ result, onReset }) => {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Hero Header Banner */}
      <GlassCard className="p-8 border border-emerald-500/40 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider font-mono">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4">
          <div className="flex items-center gap-6 bg-black/60 p-5 rounded-2xl border border-white/10 text-center font-mono">
            <div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <GlassCard className="p-6 space-y-4">
            ...
            <ul className="space-y-2.5 text-xs text-slate-300">
              {result.brief.keyInsights.map((insight, idx) => (
                <li key={idx} className="flex items-start gap-3 p-3.5 rounded-2xl bg-white/[0.04] border border-white/10">
          ...
          <GlassCard className="p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <span className="text-xs font-mono font-semibold px-2.5 py-0.5 rounded-full bg-amber-500/15 text-amber-300 border border-amber-500/30">
                TASKS CREATED
              </span>
            </div>
            ...
            <div className="space-y-2.5">
              {result.tasks.map((t, idx) => (
                <div key={t.id} className="p-3.5 rounded-2xl bg-white/[0.04] border border-white/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-xl bg-blue-500/20 text-blue-300 font-mono font-bold text-xs flex items-center justify-center border border-blue-500/30">
                      {idx + 1}
                    </span>
                    <span className="text-xs font-semibold text-slate-200">{t.title}</span>
                  </div>
                  <span className="text-[10px] font-mono font-bold uppercase px-2.5 py-1 rounded-full bg-white/10 text-slate-300">
                    {t.priority}
                  </span>
                </div>
              ))}
          ...
          {result.draftEmail && (
            <GlassCard className="p-6 border border-blue-500/40 space-y-4 relative overflow-hidden">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <div className="space-y-3 text-xs">
                ...
                {isEditingDraft ? (
                  <textarea
                    ...
                    rows={7}
                    className="w-full p-4 rounded-2xl bg-black/60 border border-blue-500/50 focus:border-cyan-400 text-slate-200 text-xs outline-none font-sans leading-relaxed resize-none"
                  />
                ) : (
                  <div className="p-4 rounded-2xl bg-[#080B10]/90 border border-white/10 font-sans text-slate-200 whitespace-pre-wrap leading-relaxed">
                    {draftBody}
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                {!isSent && (
                  <TactileButton onClick={handleSendDraft} variant="primary" className="px-5 py-2 text-xs">
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6 space-y-4">
            ...
            <div className="space-y-2 text-xs font-semibold">
              <div className="flex items-center justify-between p-2.5 rounded-2xl bg-white/[0.04] border border-white/10 text-emerald-400">
                <div className="flex items-center gap-2">
          ...
          <GlassCard className="p-5 space-y-3">
            ...
            <div className="space-y-2">
              {result.emailsFound.map((em) => (
                <div key={em.id} className="p-3 rounded-2xl bg-white/[0.04] border border-white/10 space-y-1 text-xs">
          ...
          <GlassCard className="p-5 space-y-3">
            ...
            <div className="space-y-2">
              {result.docsFound.map((doc) => (
                <div key={doc.id} className="p-3 rounded-2xl bg-white/[0.04] border border-white/10 flex items-center justify-between text-xs">
                  <div>
